...
static const u16 wm8904_reg_defaults[WM8904_MAX_REGISTER + 1] = {
...
static unsigned int wm8904_read_reg_cache(struct snd_soc_codec *codec,
						 unsigned int reg)
{
	u16 *cache = codec->reg_cache;

	BUG_ON(reg >= ARRAY_SIZE(wm8904_reg_defaults));

	return cache[reg];
}

static unsigned int wm8904_hw_read(struct snd_soc_codec *codec, u8 reg)
{
...
static unsigned int wm8904_read(struct snd_soc_codec *codec,
				unsigned int reg)
{
	if (wm8904_volatile_register(reg)) {
		return wm8904_hw_read(codec, reg);
	}
	else {
		return wm8904_read_reg_cache(codec, reg);
	}
}

static void wm8904_write_reg_cache(struct snd_soc_codec *codec,
...
static int wm8904_write(struct snd_soc_codec *codec, unsigned int reg,
			unsigned int value)
{
	u8 data[3];
...
SND_SOC_DAPM_ADC("AIFOUTL", "Capture", SND_SOC_NOPM, 0, 0),
SND_SOC_DAPM_ADC("AIFOUTR", "Capture", SND_SOC_NOPM, 0, 0),
};

static const struct snd_soc_dapm_widget wm8904_dac_dapm_widgets[] = {
SND_SOC_DAPM_DAC("AIFINL", "Playback", SND_SOC_NOPM, 0, 0),
SND_SOC_DAPM_DAC("AIFINR", "Playback", SND_SOC_NOPM, 0, 0),
...
static int wm8904_set_fll(struct snd_soc_dai *dai, int source,
			  unsigned int Fref, unsigned int Fout)
{
	...
	if (source == WM8904_FLL_FREE_RUNNING) {
	...
	case SND_SOC_BIAS_STANDBY:
		if (codec->bias_level == SND_SOC_BIAS_OFF) {
			...
			for (i = 1; i < ARRAY_SIZE(wm8904->reg_cache); i++) {
				...
				if (wm8904->reg_cache[i] == wm8904_reg_defaults[i])
					continue;
...
static int wm8904_register(struct wm8904_priv *wm8904)
{
	...
	codec->read = wm8904_read;
	codec->write = wm8904_write;
	codec->hw_write = (hw_write_t)i2c_master_send;

	memcpy(codec->reg_cache, wm8904_reg_defaults,
	       sizeof(wm8904_reg_defaults));
	...
	if (ret != 0) {
        ...
        printk("set AUDIO_CODEC_RESET_GPIO to output Low!\n");
        gpio_direction_output(AUDIO_CODEC_RESET_GPIO, 0);
        gpio_set_value(AUDIO_CODEC_RESET_GPIO, 0);

        if (gpio_request(AUDIO_CODEC_POWER_ENABLE_GPIO, "AUDIO DAC POWER ENABLE") < 0) {
                printk(KERN_ERR "can't get AUDIO_CODEC_POWER_ENABLE_GPIO \n");
                return;
        }

        printk("set AUDIO_CODEC_POWER_ENABLE_GPIO to output and value high!\n");
	...
	dev_info(codec->dev, "Device ID is %x\n", ret );
	if (ret != wm8904_reg_defaults[WM8904_SW_RESET_AND_ID]) {
