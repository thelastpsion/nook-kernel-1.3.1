...
#define AS3677_DIM_LOCK()	down(&(data)->dimming_in_progress_sema)
#define AS3677_DIM_UNLOCK()	up(&(data)->dimming_in_progress_sema)
#define AS3677_DIM_IS_LOCKED() (data->dimming_in_progress == 1)
...
{
	int i;
	struct as3677_data * const data =
	    container_of(work, struct as3677_data, shutdown_work.work);

	do_shutdown_process = false;

	AS3677_LOCK();
	/* Reset all registers to default values */
	for (i = 0; i < ARRAY_SIZE(as3677_default_data.regs); i++) {
		if (as3677_default_data.regs[i].name) {
			i2c_smbus_write_byte_data(data->client,
			i, as3677_default_data.regs[i].value);
		}
	}

	/* Do not do AS3677_UNLOCK Here. Lock-up all register access then poweroff kernel. */
	kernel_power_off();
}
	...
	unsigned int type, unsigned int code, int value)
{
	if (KEY_POWER == code) {
		struct as3677_data * const data =
			(struct as3677_data *)handle->private;
...
static int pwronkey_connect(struct input_handler *handler,
	struct input_dev *dev, const struct input_device_id *id)
{
	struct as3677_data * const data = (struct as3677_data *)handler->private;
	...
	pr_debug("as3677: Connected device: %s (%s at %s)\n",
	       dev_name(&dev->dev),
	       dev->name ?: "unknown",
	       dev->phys ?: "unknown");
...
static void pwronkey_disconnect(struct input_handle *handle)
{
	pr_debug("as3677: Disconnected device: %s\n",
...
#else
static int as3677_i2c_suspend(struct i2c_client *client, pm_message_t msg)
{
	return as3677_suspend(&client->dev);
}

static int as3677_i2c_resume(struct i2c_client *client)
{
	return as3677_resume(&client->dev);
}
#endif

static struct dev_pm_ops as3677_pm = {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 33)
...
#ifdef CONFIG_PM
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 33)
	.suspend = as3677_i2c_suspend,
	.resume = as3677_i2c_resume,
#endif
#endif
};
		...
		enum led_brightness value)
{
	...
	as3677_set_brightness(data, &data->leds[0], value);
#else
		...
		const char *buf)
{
	...
	i = sscanf(buf,
		" %hhu, %hhu, %hhu,   %d, %hhu, %hhu"
		":%hhu: %hhu, %hhu, %hhd, %hhu, %hhd, %hhu, %hhu"
		":%hhu: %hhu, %hhu, %hhd, %hhu, %hhd, %hhu, %hhu"
		":%hhu: %hhu, %hhu, %hhd, %hhu, %hhd, %hhu, %hhu"
		, rh, rl, gp, sg, ga, o
				...
				struct device_attribute *attr, char *buf)
{
	...
	MSNPRINTF("#rh,rl,gp,sgm,ga,o[:gn:y0, x1, k1, x2, k2, y3, ss]{1-3} \n");
	MSNPRINTF("#|                                                      \n");
		...
		struct as3677_data *data, struct as3677_platform_data *pdata)
{
	...
	if (err)
		goto exit;
#if 0 //No need for setting brightness here.
      //This causes front light to turn on during bootup.
	for (i = 0; i < data->num_leds; i++) {
		struct as3677_led *led = &data->leds[i];
		...
		const struct i2c_device_id *id)
{
	...
	*data = as3677_default_data;
	...
	dev_info(&client->dev, "AS3677 driver v1.4: detected AS3677"
			...
			id1, id2);
	if (as3677_pdata->pwronkey_shutdown_msec > 0) {
		int ret;
		data->shutdown_timer_msec = as3677_pdata->pwronkey_shutdown_msec;
