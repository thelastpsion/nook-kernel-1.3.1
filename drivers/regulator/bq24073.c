...
static int bq24073_set_suspend_enable(struct regulator_dev *rdev)
{
    ...
    if (pdata->gpio_nce_state) {
        ret |= gpio_direction_input(pdata->gpio_nce);
        ret |= gpio_direction_input(pdata->gpio_en1);
        ret |= gpio_direction_input(pdata->gpio_en2);
	//Regulator core never calls bq24073_set_suspend_disable(), so we initialize the gpio pin once
	//usb cable is connected again.
	pdata->init_nce = 0;
	pdata->init_en = 0;
    } else {
