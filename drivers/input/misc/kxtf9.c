...
#define TAP_TIMER     0x2E
#define TOTAL_TIMER   0x2F
#define LAT_TIMER     0x30
#define WIN_TIMER     0x31
...
#define RES_TILT_ANGLE      14
#define RES_HYST_SET        15
...
struct kxtf9_data {
    ...
    struct delayed_work baseline_work;
    int baseline_poll;
    int baseline_count;
    ...
    int polling;
	u8 who_am_i;
	wait_queue_head_t tap_state_wq;
	int tap_state,last_tap_state;
    unsigned long tilt_jifies;
    u8 tilt_old;
    int device_resume_mode;
...
//	err = kxtf9_i2c_write(tf9, CTRL_REG1, buf, 1);
    ...
    err = kxtf9_i2c_write(tf9, TILT_ANGLE, &tf9->resume[RES_TILT_ANGLE], 1);
    if (err < 0)
        return err;
    aprintk("kxtf9: kxtf9_hw_init > kxtf9 resume TILT_ANGLE!\n");

    err = kxtf9_i2c_write(tf9, HYST_SET, &tf9->resume[RES_HYST_SET], 1);
    if (err < 0)
        return err;
    aprintk("kxtf9: kxtf9_hw_init > kxtf9 resume HYST_SET!\n");
 ...
 */
	...
	if ((status & TPS) > 0) {
        ...
        elapsed_tilt = jiffies_to_msecs(jiffies-tf9->tilt_jifies);

        aprintk("tilt 0x%02x  was 0x%02x elapsed %d mSec\n",buf[0],buf[1],elapsed_tilt);

        if (tf9->tilt_jifies>0) {
            
            if (tf9->post_key_tilt >7 || 
                (tf9->post_key_tilt==1 && (buf[0]&0x30))
                || (tf9->post_key_tilt==2 && (buf[0]&0x0c))
                || (tf9->post_key_tilt==3 && (buf[0]&0x03)) ) {
                aprintk("post page turn\n");
                input_event(tf9->input_dev, EV_KEY, KEY_NEXT, !!1);
                input_event(tf9->input_dev, EV_KEY, KEY_NEXT, !1);
                input_sync(tf9->input_dev);
            }

            if (tf9->post_key_tilt == 4 || tf9->post_key_tilt == 5) {
                if (tf9->tilt_old == buf[0] && elapsed_tilt<1500) {
                    aprintk("post page turn\n");
                    input_event(tf9->input_dev, EV_KEY, KEY_NEXT, !!1);
                    input_event(tf9->input_dev, EV_KEY, KEY_NEXT, !1);
                    input_sync(tf9->input_dev);
            ...
            if (tf9->baseline_poll) {
                tf9->baseline_count = 1;
                cancel_delayed_work_sync(&tf9->baseline_work);
                schedule_delayed_work(&tf9->baseline_work, msecs_to_jiffies(tf9->baseline_poll));
            }

        } 
        tf9->tilt_jifies = jiffies;
        tf9->tilt_old = buf[1];
	...
	if (((status & TDTS0) | (status & TDTS1)) > 0) {
		...
		tf9->tap_state = buf[0];
		tf9->last_tap_state = buf[0];
		wake_up(&tf9->tap_state_wq);
		/*** <end> DEBUG OUTPUT - REMOVE ***/
		// zz 
        if (tf9->post_key_tap >3 || 
            (tf9->post_key_tap==1 && (buf[0]&0x30))
            || (tf9->post_key_tap==2 && (buf[0]&0x0c))
            || (tf9->post_key_tap==3 && (buf[0]&0x03)) ) {
        		input_event(tf9->input_dev, EV_KEY, KEY_NEXT, !!1);
        		input_event(tf9->input_dev, EV_KEY, KEY_NEXT, !1);
        		input_sync(tf9->input_dev);
        }
...
int kxtf9_update_odr(struct kxtf9_data *tf9, int poll_interval)
{
	...
	if (atomic_read(&tf9->enabled)) {
		...
		if (err < 0)
			return err;
        aprintk("Kionix data control reg set to 0x%02x\n",config);
		/*
...
static void kxtf9_baseline_work_func(struct work_struct *work)
{
    struct kxtf9_data *tf9 = container_of((struct delayed_work *)work,
                                          struct kxtf9_data, baseline_work);
    int xyz[3] = { 0};
    long m1,m2;
    int changed=0;
    int zscale;
    ...
    if (err == 0) {
        m1=((int)xyz[0])*xyz[0] + ((int)xyz[1]*xyz[1]);
        m2=((int)xyz[2])*xyz[2];
        aprintk("delayed baseline --- accel %d %d %d  -- mr = %d  mz=%d\n",xyz[0],xyz[1],xyz[2],m1,m2);
       
        zscale = (tf9->resume[RES_TILT_ANGLE]==25)?1:4;

            if (zscale* m2/2<m1) {
                aprintk("should adjust hyst tilt to 25\n");
                if (tf9->post_key_tilt == 5) {
                    changed=25;
                }

            } else {
                aprintk("should adjust tilt to 10\n");
                if (tf9->post_key_tilt == 5) {
                    changed=10;
                }
            }
        ...
        if (changed) {
            u8 val2,val3;
            if (tf9->resume[RES_TILT_ANGLE] != changed) {
                tf9->resume[RES_TILT_ANGLE] = changed;
                mutex_lock(&tf9->lock);
                kxtf9_i2c_read(tf9, INT_CTRL1, &val2, 1);
                val3 = val2 & 0x7f;
                kxtf9_i2c_write(tf9, INT_CTRL1, &val3, 1);
                kxtf9_i2c_write(tf9, TILT_ANGLE, &tf9->resume[RES_TILT_ANGLE], 1);
                kxtf9_i2c_write(tf9, INT_CTRL1, &val2, 1);
...
static void kxtf9_input_work_func(struct work_struct *work)
{
	...
	if (tf9->polling) {
		...
		schedule_delayed_work(&tf9->input_work, msecs_to_jiffies(tf9->res_interval));
	}
	mutex_unlock(&tf9->lock);
}
...
static int kxtf9_input_init(struct kxtf9_data *tf9)
{
    ...
    INIT_DELAYED_WORK(&tf9->baseline_work, kxtf9_baseline_work_func);
    ...
    tf9->input_dev->id.bustype = BUS_HOST;
    tf9->input_dev->id.vendor = 0x0301;
    tf9->input_dev->id.product = 0x0002;
    tf9->input_dev->id.version = 0x0200;

    __set_bit(EV_KEY, tf9->input_dev->evbit);
    __set_bit(EV_REP, tf9->input_dev->evbit);
    ...
    //    input_set_capability(tf9->input_dev, EV_KEY, button->code);
    ...
    input_set_capability(tf9->input_dev, EV_KEY, KEY_VOLUMEDOWN);
	input_set_capability(tf9->input_dev, EV_KEY, KEY_VOLUMEUP);
    input_set_capability(tf9->input_dev, EV_KEY, KEY_NEXT);
    input_set_capability(tf9->input_dev, EV_KEY, KEY_BACK);
    input_set_capability(tf9->input_dev, EV_KEY, KEY_PREVIOUS);
    input_set_capability(tf9->input_dev, EV_KEY, KEY_MENU);

#if 0
	input_event(input, EV_KEY, KEY_NEXT, !!1);
	input_event(input, EV_KEY, KEY_NEXT, !1);
	input_sync(input);
#endif
				...
				struct device_attribute *attr, char *buf)
{
    ...
    aprintk("tf9->resume[RES_DATA_CTRL] = 0x%02x",tf9->resume[RES_DATA_CTRL]);
    n=sprintf(buf,"tf9->resume[RES_DATA_CTRL] = 0x%02x",tf9->resume[RES_DATA_CTRL]);
...
static ssize_t kxtf9_polling_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kxtf9_data *tf9 = i2c_get_clientdata(client);
    int n;
    n=sprintf(buf,"tf9->polling = 0x%02x",tf9->polling);
	return n;
}

static ssize_t kxtf9_polling_store(struct device *dev,
					struct device_attribute *attr,
						const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
				...
				struct device_attribute *attr, char *buf)
{
	...
	kxtf9_i2c_read(tf9, CTRL_REG2, &z, 1);
	aprintk("CTRL_REG2 = 0x%02x\n",z);
	kxtf9_i2c_read(tf9, INT_CTRL_REG2, &z, 1);
	aprintk("INT_CTRL_REG2 = 0x%02x\n",z);
...
static ssize_t kxtf9_wait_for_tap_event_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kxtf9_data *tf9 = i2c_get_clientdata(client);
	...
	ret = wait_event_interruptible(tf9->tap_state_wq,
				       tf9->tap_state != 0);
	s += sprintf(buf, "%d",tf9->tap_state);
	tf9->tap_state = 0;
	return s - buf;
}


static ssize_t kxtf9_tap_state_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kxtf9_data *tf9 = i2c_get_clientdata(client);
	...
	s += sprintf(buf, "0x%02x",tf9->last_tap_state);
	tf9->last_tap_state = 0;
	return s - buf;
}

static ssize_t kxtf9_post_tap_key_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kxtf9_data *tf9 = i2c_get_clientdata(client);
...
static ssize_t kxtf9_post_tap_key_store(struct device *dev,
					struct device_attribute *attr,
						const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kxtf9_data *tf9 = i2c_get_clientdata(client);
	tf9->post_key_tap = simple_strtoul(buf, NULL, 10);

	return count;
}
static DEVICE_ATTR(post_tap_key, S_IRUGO|S_IWUSR, kxtf9_post_tap_key_show, kxtf9_post_tap_key_store);

static ssize_t kxtf9_post_tilt_key_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kxtf9_data *tf9 = i2c_get_clientdata(client);
...
static ssize_t kxtf9_post_tilt_key_store(struct device *dev,
					struct device_attribute *attr,
						const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct kxtf9_data *tf9 = i2c_get_clientdata(client);
	tf9->post_key_tilt = simple_strtoul(buf, NULL, 10);

	return count;
}
static DEVICE_ATTR(post_tilt_key, S_IRUGO|S_IWUSR, kxtf9_post_tilt_key_show, kxtf9_post_tilt_key_store);





static DEVICE_ATTR(wait_for_tap, S_IRUGO, kxtf9_wait_for_tap_event_show, NULL);

static DEVICE_ATTR(tap_state, S_IRUGO, kxtf9_tap_state_show, NULL);
...
static DEVICE_ATTR(polling, S_IRUGO|S_IWUGO, kxtf9_polling_show, kxtf9_polling_store);


static ssize_t kxtf9_register_store(struct device *dev,
					struct device_attribute *attr,
						const char *buf, size_t count);

static ssize_t kxtf9_register_show(struct device *dev,
				struct device_attribute *attr, char *buf);

#define kxtf9_register_attr(x)    static DEVICE_ATTR( reg_##x, S_IRUGO|S_IWUGO, kxtf9_register_show, kxtf9_register_store)

kxtf9_register_attr(TDT_TIMER);
kxtf9_register_attr(TDT_H_THRESH);
kxtf9_register_attr(TDT_L_THRESH);
kxtf9_register_attr(TAP_TIMER);
kxtf9_register_attr(TOTAL_TIMER);
kxtf9_register_attr(LAT_TIMER);
kxtf9_register_attr(WIN_TIMER);

kxtf9_register_attr(DATA_CTRL);
kxtf9_register_attr(CTRL_REG1);
kxtf9_register_attr(INT_CTRL1);
kxtf9_register_attr(TILT_TIMER);
kxtf9_register_attr(CTRL_REG3);
kxtf9_register_attr(WUF_TIMER);
kxtf9_register_attr(WUF_THRESH);

kxtf9_register_attr(TILT_ANGLE);
kxtf9_register_attr(HYST_SET);
...
static struct attribute *kxtf9_attributes[] = {
	...
	&dev_attr_wait_for_tap.attr,
	&dev_attr_tap_state.attr,
    &dev_attr_post_tap_key.attr,
    &dev_attr_post_tilt_key.attr,
    &dev_attr_polling.attr,
	&dev_attr_reg_TDT_TIMER.attr,
	&dev_attr_reg_TDT_H_THRESH.attr,
	&dev_attr_reg_TDT_L_THRESH.attr,
	&dev_attr_reg_TAP_TIMER.attr,
	&dev_attr_reg_TOTAL_TIMER.attr,
	&dev_attr_reg_LAT_TIMER.attr,
	&dev_attr_reg_WIN_TIMER.attr,
    &dev_attr_reg_WUF_TIMER.attr,
    &dev_attr_reg_WUF_THRESH.attr,
    &dev_attr_reg_DATA_CTRL.attr,
    &dev_attr_reg_CTRL_REG1.attr,
    &dev_attr_reg_INT_CTRL1.attr,
    &dev_attr_reg_TILT_TIMER.attr,
    &dev_attr_reg_CTRL_REG3.attr,
    &dev_attr_reg_TILT_ANGLE.attr,
    &dev_attr_reg_HYST_SET.attr,
	NULL
};
						...
						const char *buf, size_t count)
{
	...
	aprintk("kxtf9: kxtf9_register_store ...\n");
	
#define  KXTF9_REGISTER_CHECK_STORE(x) \
	if ( attr == (struct device_attribute *) &dev_attr_reg_##x.attr  ) { regaddr=x; offset=RES_##x; aprintk("set register %s (0x%02x) = 0x%02x\n", #x , regaddr, val); }

	KXTF9_REGISTER_CHECK_STORE(TDT_TIMER);
	KXTF9_REGISTER_CHECK_STORE(TDT_H_THRESH);
	KXTF9_REGISTER_CHECK_STORE(TDT_L_THRESH);
	KXTF9_REGISTER_CHECK_STORE(TAP_TIMER);
	KXTF9_REGISTER_CHECK_STORE(TOTAL_TIMER);
	KXTF9_REGISTER_CHECK_STORE(LAT_TIMER);
	KXTF9_REGISTER_CHECK_STORE(WIN_TIMER);

    KXTF9_REGISTER_CHECK_STORE(WUF_TIMER);
    KXTF9_REGISTER_CHECK_STORE(WUF_THRESH);
    KXTF9_REGISTER_CHECK_STORE(DATA_CTRL);
    KXTF9_REGISTER_CHECK_STORE(CTRL_REG1);
    KXTF9_REGISTER_CHECK_STORE(INT_CTRL1);
    KXTF9_REGISTER_CHECK_STORE(TILT_TIMER);
    KXTF9_REGISTER_CHECK_STORE(CTRL_REG3); 
    KXTF9_REGISTER_CHECK_STORE(TILT_ANGLE); 
    KXTF9_REGISTER_CHECK_STORE(HYST_SET); 
    if (offset!=-1) {
        if (regaddr!=INT_CTRL1) {
            kxtf9_i2c_read(tf9, INT_CTRL1, &val2, 1);
            val3 = val2 & 0x7f;
            kxtf9_i2c_write(tf9, INT_CTRL1, &val3, 1);
        }
	  kxtf9_i2c_write(tf9, regaddr, &val, 1);
	  tf9->resume[offset]=val;
      if (regaddr!=INT_CTRL1) {
          kxtf9_i2c_write(tf9, INT_CTRL1, &val2, 1);
      }
				...
				struct device_attribute *attr, char *buf)
{
    ...
    regname[0]='\0';
#define  KXTF9_REGISTER_GET_OFFSET(x) \
	if (attr == (struct device_attribute *) &dev_attr_reg_##x.attr ) {  strcpy(regname,#x); regaddr=x;   offset=RES_##x; aprintk("get register " #x " (0x%02x) = 0x%02x (cached)\n", regaddr, tf9->resume[offset]); }
	
	KXTF9_REGISTER_GET_OFFSET(TDT_TIMER);
	KXTF9_REGISTER_GET_OFFSET(TDT_H_THRESH);
	KXTF9_REGISTER_GET_OFFSET(TDT_L_THRESH);
	KXTF9_REGISTER_GET_OFFSET(TAP_TIMER);
	KXTF9_REGISTER_GET_OFFSET(TOTAL_TIMER);
	KXTF9_REGISTER_GET_OFFSET(LAT_TIMER);
	KXTF9_REGISTER_GET_OFFSET(WIN_TIMER);

    KXTF9_REGISTER_GET_OFFSET(WUF_TIMER);
    KXTF9_REGISTER_GET_OFFSET(WUF_THRESH);
    KXTF9_REGISTER_GET_OFFSET(DATA_CTRL);
    KXTF9_REGISTER_GET_OFFSET(CTRL_REG1);
    KXTF9_REGISTER_GET_OFFSET(INT_CTRL1);
    KXTF9_REGISTER_GET_OFFSET(TILT_TIMER);
    KXTF9_REGISTER_GET_OFFSET(CTRL_REG3);
    KXTF9_REGISTER_GET_OFFSET(TILT_ANGLE);
    KXTF9_REGISTER_GET_OFFSET(HYST_SET);


    if (offset!=-1) {
	  u8 buffer;
	  err = kxtf9_i2c_read(tf9, regaddr, &buffer, 1);
	  if (!err) 
	    s += sprintf(buf, "reg 0x%02x (%s) = 0x%02x\n",regaddr, regname, buffer);
	  else
	    s += sprintf(buf, "error reading i2c bus\n");
	}
	else
	  s += sprintf(buf,"No attribute found\n");
	return s - buf;
}
						...
						const struct i2c_device_id *id)
{
	...
	init_waitqueue_head(&tf9->tap_state_wq);
	tf9->tap_state = 0;
	tf9->last_tap_state = 0;
    ...
    tf9->resume[RES_TILT_ANGLE] = tf9->pdata->tilt_angle_init;
    tf9->resume[RES_HYST_SET]    = tf9->pdata->hyst_set_init;

    tf9->post_key_tap = tf9->pdata->post_tap_mode;
    tf9->post_key_tilt = tf9->pdata->post_tilt_mode;
    tf9->polling = tf9->pdata->polling;
    tf9->baseline_poll = tf9->pdata->baseline_poll;
...
static int kxtf9_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	tf9->device_resume_mode = tf9->hw_initialized;
