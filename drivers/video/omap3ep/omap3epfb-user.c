...
// Are we doing a GLR or GLRD update?
static int mInGLRDMode = 0;

static inline void reset_effect_areas(struct omap3epfb_par *par)
{
	par->effect_active = 0;
...
static void omap3epfb_disable_work(struct work_struct *work)
{
	...
	DEBUG_LOG(DEBUG_LEVEL2,"delayed enable EPD Updates (%s)\n",
	          (mInGLRDMode == OMAP3EPFB_WVFID_GLR) ? "GLR" :
	          (mInGLRDMode == OMAP3EPFB_WVFID_GLRD) ? "GLRD" :
	          (par->next_delay_GC) ? "GC" : "AUTO");

	omap3epfb_reqq_purge(par->info);
	if (mInGLRDMode != 0) {
		omap3epfb_update_screen(par->info, mInGLRDMode, false);
	}
	else if (par->next_delay_GC) {
		omap3epfb_update_screen(par->info, OMAP3EPFB_WVFID_GC, false);
	}
	else {
		area = &par->effect_array[FIRST_USER_REGION];
		if ((area->effect_area.x0 == 0) && (area->effect_area.x1 == 0) &&
		    (area->effect_area.y0 == 0) && (area->effect_area.y1 == 0)) {
			omap3epfb_update_screen(par->info, OMAP3EPFB_WVFID_AUTO, false);
		}
		else {
			omap3epfb_update_area(par->info, &area->effect_area);
		}
		omap3epfb_reset_region(par->info, (1 << FIRST_USER_REGION));
		par->next_delay_GC = true; // Reset to default
	}
	g_disable_time = 0;  // Reset expiration time
	par->disable_time_ms = 0;
}
...
static void rect_dump(struct omap3epfb_update_area *a)
{
	...
	static const char *wvfm_lf_mode = "LF";
	static const char *wvfm_nf_mode = "NF";
	...
	if (a->wvfid == OMAP3EPFB_WVFID_VU)
		txt = wvfm_vl_mode;
	if (a->wvfid == OMAP3EPFB_WVFID_GLR)
		txt = wvfm_nf_mode;
	if (a->wvfid == OMAP3EPFB_WVFID_GLRD)
		txt = wvfm_lf_mode;
...
static void update_effect(struct omap3epfb_par *par, int index)
{
	...
	static const char wvfid_char_vu[1] = "V";  // VU mode
	static const char wvfid_char_lf[1] = "F";  // Low-flash mode
	static const char wvfid_char_nf[1] = "N";  // No-flash mode
	struct omap3epfb_area *area = NULL;
	...
	if (area->effect_flags)
	{
		...
		else if (area->effect_area.wvfid == OMAP3EPFB_WVFID_GLR)
			par->effect_active_debug[index] = wvfid_char_nf[0];
		else if (area->effect_area.wvfid == OMAP3EPFB_WVFID_GLRD)
			par->effect_active_debug[index] = wvfid_char_lf[0];
		else
...
static int process_area(int index, struct fb_info *info, struct omap3epfb_area *area, struct omap3epfb_update_area *p)
{
	...
	res = rect_inside(&area->effect_area, p);
	if (!res)
	{
	...
	{
		DEBUG_REGION(DEBUG_LEVEL5, p,"Match 0x%02x region %d = ", area->effect_flags, index);
	}
	...
	if (area->effect_flags & EFFECT_ONESHOT)
	{
		...
		if ((area->effect_area.wvfid == OMAP3EPFB_WVFID_GLR) ||
		    (area->effect_area.wvfid == OMAP3EPFB_WVFID_GLRD)) {
			mInGLRDMode = 0;
		}
	...
	else if ((area->effect_flags & EFFECT_CLEAR) && rect_equal(&area->effect_area, p))
	{
		...
		p->x1 = info->var.xres-1;
		p->y1 = info->var.yres-1;
		if ((p->wvfid == OMAP3EPFB_WVFID_GLR) ||
		    (p->wvfid == OMAP3EPFB_WVFID_GLRD)) {
			mInGLRDMode = 0;
		}
		p->wvfid = OMAP3EPFB_WVFID_GC;
		par->effect_array[index].effect_flags = 0;
...
int user_update(struct fb_info *info, struct omap3epfb_update_area *p)
{
	...
	// If EPD is in GLR/D mode, must do a full-screen update
	if (mInGLRDMode != 0) {
		DEBUG_LOG(DEBUG_LEVEL3, "full-screen GLR/D update\n");
		omap3epfb_update_screen(par->info, mInGLRDMode, false);
		return 0;
	}
...
int omap3epfb_set_region(struct fb_info *info, struct omap3epfb_area *p)
{
	...
	// Sometimes the Apps try to set an EPD region before the view is up; if so, ignore request
	if ((p->effect_area.x0 == 0) && (p->effect_area.x1 == 0) &&
	    (p->effect_area.y0 == 0) && (p->effect_area.y1 == 0)) {
		DEBUG_LOG(DEBUG_LEVEL4,"WARNING: Not setting region %d because all coordinates are zero\n", p->index);
		return 0;
	}

	if (mInGLRDMode != 0) {
		if ( (p->effect_flags) &&
		     (p->effect_area.wvfid != OMAP3EPFB_WVFID_GLR) &&
	             (p->effect_area.wvfid != OMAP3EPFB_WVFID_GLRD)) {
			DEBUG_LOG(DEBUG_LEVEL4,"WARNING: Not setting region %d because we are in GLR[D] mode\n", p->index);
		}
		return 0;
	}
	...
	if ( (p->effect_flags) && 
	     ((p->effect_area.wvfid == OMAP3EPFB_WVFID_GLR) ||
	      (p->effect_area.wvfid == OMAP3EPFB_WVFID_GLRD))) {
		DEBUG_LOG(DEBUG_LEVEL4,"Clearing all regions prior to setting GLR[D]\n");
		reset_effect_areas(par);
		// GLR[D] updates need to be done on the entire screen, regardless of
		// the area requested
		p->effect_area.x0 = p->effect_area.y0 = 0;
		p->effect_area.x1 = info->var.xres;
		p->effect_area.y1 = info->var.yres;
		mInGLRDMode = p->effect_area.wvfid;
	}
...
int omap3epfb_reset_region(struct fb_info *info, int mask)
{
	...
	mutex_lock(&par->area_mutex);
	{
		...
		for (index = 0; index < EFFECT_ARRAY_SIZE; index++)
		{
			if (mask & (1 << index))
			{
				p = &par->effect_array[index];
				if ( (p->effect_flags) &&
				     ((p->effect_area.wvfid == OMAP3EPFB_WVFID_GLR) ||
				      (p->effect_area.wvfid == OMAP3EPFB_WVFID_GLRD))) {
					// Clearing a GLR/D area
					mInGLRDMode = 0;
					DEBUG_LOG(DEBUG_LEVEL4,"Turning GLR/D mode OFF\n");
				}
				// Clear area
			  ...
			  const char *buf, size_t count)
{
	...
	switch (clear)
	{
		...
		// Clear the frame buffer to white and force GC update
		case 3: memset(fb->screen_base, 0xff, 4 * fb->var.xres * fb->var.yres);
		        printk("epd_refresh: About to clear screen...\n");
			  ...
			  const char *buf, size_t count)
{
	...
	// Special value of 0 is used to set the flag for GC update next time
	if (tmp == 0) {
		par->next_delay_GC = true;
		DEBUG_LOG(DEBUG_LEVEL4,"Next EPD delayed update will force GC\n");
		return count;
	}
	// Special value of 1 is used to indicate AUTO update next time
	else if (tmp == 1) {
		par->next_delay_GC = false;
		DEBUG_LOG(DEBUG_LEVEL4,"Next EPD delayed update will force AUTO\n");
		return count;
	}
	// Special value of 2 is used to temporarily disable the delayed EPD update function
	else if (tmp == 2) {
		par->ignore_delays = true;
		cancel_delayed_work_sync(&par->clear_work);
		cancel_delayed_work_sync(&par->disable_work);
		g_disable_time = 0;  // Reset expiration time
		par->disable_time_ms = 0;
		DEBUG_LOG(DEBUG_LEVEL4,"Disabling EPD delayed updates\n");
		return count;
	}
	// Special value of 3 is used to re-enable the delayed EPD update function
	else if (tmp == 3) {
		par->ignore_delays = false;
		par->disable_time_ms = 0;
		DEBUG_LOG(DEBUG_LEVEL4,"Re-enabling EPD delayed updates\n");
		return count;
	}
	// Special value of 4 is used to temporarily disable all EPD updates
	// Re-enable EPD updates with a call to this routine using special value 3
	else if (tmp == 4) {
		par->ignore_delays = true;
		cancel_delayed_work_sync(&par->clear_work);
		cancel_delayed_work_sync(&par->disable_work);
		g_disable_time = 0;
		par->disable_time_ms = 1;  // This value will stop any coming updates
		DEBUG_LOG(DEBUG_LEVEL4,"Disabling EPD updates\n");
		return count;
	}

	if (par->ignore_delays == true) {
		DEBUG_LOG(DEBUG_LEVEL4,"EPD delayed updates are disabled; ignoring request\n");
		return count;
	}

	par->disable_time_ms = tmp;
	...
	if (tmp > 0)
	{
		...
		t_expires += (u32)par->disable_time_ms;
		if (t_expires > g_disable_time)
		{
			cancel_delayed_work_sync(&par->clear_work);
			cancel_delayed_work_sync(&par->disable_work);
			par->disable_time_ms = tmp; /* again, in case disable_work just ran */
			schedule_delayed_work(&par->disable_work, msecs_to_jiffies(par->disable_time_ms));
				...
				struct device_attribute *attr, char *buf)
{
	...
	unsigned char cpld_hw_ver = gpio_get_value(CPLD_V_DET1_GPIO) |
		       (gpio_get_value(CPLD_V_DET2_GPIO) << 1) |
		       (gpio_get_value(CPLD_V_DET3_GPIO) << 2) |
		       (gpio_get_value(CPLD_V_DET4_GPIO) << 3);
#else // Gossamer and Owl
	unsigned char cpld_hw_ver = gpio_get_value(CPLD_V_DET1_GPIO) |
		       (gpio_get_value(CPLD_V_DET2_GPIO) << 1);
#endif
...
int user_init_device(struct fb_info *fb_info)
{
	...
	// Enable the delayed update function
	par->ignore_delays = false;

	// Use GC for delayed updates unless requested to do AUTO
	par->next_delay_GC = true;

	// Clear all effect areas
	reset_effect_areas(par);
