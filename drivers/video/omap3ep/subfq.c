...
extern struct wake_lock epd_pmic_wakelock;
				...
				const struct omap3epfb_update_area *src_area)
{
	...
	area = *src_area;
	dev_dbg(info->device, "==> ENTER: area.wvfid = %d; g_current_wvfid = %d\n", area.wvfid, g_current_wvfid);

	// Prevent the system from suspending while we update the EPD
	//Assuming 4Sec should be enough to update screen, if it takes
        //more then someting is wrong and wake_lock should timeout.
	...
	dev_dbg(info->device, "==> EXIT: g_current_wvfid = %d\n", g_current_wvfid);
...
bool omap3epfb_is_bq_empty(struct fb_info *info)
{
	struct omap3epfb_par *par = info->par;
	bool st = 0;

	spin_lock(&par->bfb.que.xi_lock);
	st = (par->subfq.shrd.p->rq.bqwi == par->subfq.shwr.p->wq.bqri);
	spin_unlock(&par->bfb.que.xi_lock);
	return st;
}
