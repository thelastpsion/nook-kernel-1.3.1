...
struct wake_lock epd_pmic_wakelock;
					...
					unsigned int vcomoff_time_ms)
{
	...
	*sess = kzalloc(sizeof(**sess), GFP_KERNEL);
	...
	wake_lock_init(&epd_pmic_wakelock, WAKE_LOCK_SUSPEND, "epd_pmic");
...
void pmic_remove(struct pmic_sess **sess)
{
	...
	wake_lock_destroy(&epd_pmic_wakelock);
...
int pmic_req_powerup(struct pmic_sess *sess)
{
	...
	// Prevent the system from suspending while we update the EPD
	//Assuming 4Sec to finish upadte after power up request
	wake_lock_timeout(&epd_pmic_wakelock,4*HZ);
	...
	stat = sess->drv->hw_power_ack(sess);
	if (stat) {
		int i;
...
int pmic_sync_powerup(struct pmic_sess *sess)
{
	...
	if (sess->drv->hw_power_ack(sess) <= 0) {
		int i;
...
void pmic_release_powerup_req(struct pmic_sess *sess)
{
	...
	// Prevent the system from suspending while we power off the EPD,
	// and wait for the rails to settle
	wake_lock_timeout(&epd_pmic_wakelock, timeout_jif +
                          PAPYRUS_STANDBY_DWELL_TIME * HZ + 10);
}
