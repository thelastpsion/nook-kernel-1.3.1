...
DEFINE_MUTEX(papyrus_sysfs_mutex);
...
#define HW_POWERUP_TIMEOUT   msecs_to_jiffies(50)
...
struct papyrus_sess {
	...
	struct completion  powerup_done;
	bool waiting_for_powerup;
	int power_ready_irq;
...
#define VCOM_CTRL_GPIO		(84)	/* active high */
#define PWR_GOOD_GPIO		(86)	/* active high */
...
#define STANDBY_MASK	0x40

#define PAPYRUS_HIGH_VOL_PWRDN_DELAY_MS 100

#define CPLD_POWER_RAMPUP_TIME 10
...
static uint8_t papyrus2_i2c_addr = 0;
...
static int papyrus_hw_getreg(struct papyrus_sess *sess, uint8_t regaddr, uint8_t *val)
{
	...
	if (stat < 0)
	{
		pr_err("papyrus: I2C read error: %d (I2C addr: 0x%x)\n", stat,
                        papyrus2_i2c_addr);
		pr_err("error in %s\n", __FILE__); 
	}
	else if (stat != ARRAY_SIZE(msgs)) {
...
static irqreturn_t papyrus_sess_irq_handler(int irq, void *dev)
{
	struct papyrus_sess *sess = dev;

	sess->waiting_for_powerup = false;
	complete(&sess->powerup_done);
	return IRQ_HANDLED;
}


static int papyrus_synchronized_wait_for_powerup_timeout(struct completion *res)
{
	int ret = 0;

	mutex_lock(&papyrus_sysfs_mutex);
	ret = wait_for_completion_timeout(res, HW_POWERUP_TIMEOUT);
	mutex_unlock(&papyrus_sysfs_mutex);
...
static void papyrus_hw_send_powerup(struct papyrus_sess *sess)
{
	...
	// Initialize power up flags
	init_completion(&sess->powerup_done);
	sess->waiting_for_powerup = true;

	/* enable CPLD */
	gpio_direction_output(CPLD_RESET_GPIO, 0);
	gpio_direction_output(EN_CPLD_POW_GPIO, 1);
	msleep(CPLD_POWER_RAMPUP_TIME);
	gpio_direction_output(CPLD_RESET_GPIO, 1);

	/* Make sure VCOM is OFF */
	gpio_direction_output(VCOM_CTRL_GPIO, 0);
	...
	sess->enable_reg_shadow &= ~STANDBY_MASK;
...
static int papyrus_detect_i2c_address(struct papyrus_sess *sess)
{
	...
	// Try the other address (earlier version; less likely)
...
static int papyrus_hw_init(struct papyrus_sess *sess, const char *chip_id)
{
	...
	ret |= stat;
	if (stat) pr_err("gpio_request(WAKEUP_GPIO) failed\n");
	stat = gpio_request(EN_CPLD_POW_GPIO, "cpld-pwr");
	ret |= stat;
	if (stat) pr_err("gpio_request(EN_CPLD_POW_GPIO) failed\n");
	stat = gpio_request(CPLD_RESET_GPIO, "cpld-reset");
	ret |= stat;
	if (stat) pr_err("gpio_request(CPLD_RESET_GPIO) failed\n");
	stat = gpio_request(CPLD_V_DET1_GPIO, "cpld_hw_rev");
	ret |= stat;
	if (stat) pr_err("gpio_request(CPLD_V_DET1_GPIO) failed\n");
	stat = gpio_request(CPLD_V_DET2_GPIO, "cpld_hw_rev");
	ret |= stat;
	if (stat) pr_err("gpio_request(CPLD_V_DET2_GPIO) failed\n");
#if defined(CONFIG_MACH_OMAP3621_AVOCET)
	stat = gpio_request(CPLD_V_DET3_GPIO, "cpld_hw_rev");
	ret |= stat;
	if (stat) pr_err("gpio_request(CPLD_V_DET3_GPIO) failed\n");
	stat = gpio_request(CPLD_V_DET4_GPIO, "cpld_hw_rev");
	ret |= stat;
	if (stat) pr_err("gpio_request(CPLD_V_DET4_GPIO) failed\n");
#endif
	stat = gpio_request(PWR0_GPIO, "papyrus_pwr0");	/* en_epd_pwrup */
	ret |= stat;
	if (stat) pr_err("gpio_request(PWR0_GPIO) failed\n");
	stat = gpio_request(VCOM_CTRL_GPIO, "papyrus_vcom_ctrl");
	ret |= stat;
	if (stat) pr_err("gpio_request(VCOM_CTRL_GPIO) failed\n");
	stat = gpio_request(PWR_GOOD_GPIO, "papyrus_pwr_good");
	ret |= stat;
	if (stat) pr_err("gpio_request(PWR_GOOD_GPIO) failed\n");

	if (ret) {
		pr_err("Papyrus tps65185: cannot reserve GPIOs\n");
		stat = -ENODEV;
		return stat;
	}
	...
	/* CPLD HW revision GPIOs */
	gpio_direction_input(CPLD_V_DET1_GPIO);
	gpio_direction_input(CPLD_V_DET2_GPIO);
#if defined(CONFIG_MACH_OMAP3621_AVOCET)
	gpio_direction_input(CPLD_V_DET3_GPIO);
	gpio_direction_input(CPLD_V_DET4_GPIO);
#endif
	...
	/* The power good GPIO indicates that all 4 rails are OK */
	gpio_direction_input(PWR_GOOD_GPIO);
...
free_gpios:
	...
	gpio_free(PWR0_GPIO);
	gpio_free(VCOM_CTRL_GPIO);
	gpio_free(PWR_GOOD_GPIO);
	gpio_free(CPLD_V_DET1_GPIO);
	gpio_free(CPLD_V_DET2_GPIO);
#if defined(CONFIG_MACH_OMAP3621_AVOCET)
	gpio_free(CPLD_V_DET3_GPIO);
	gpio_free(CPLD_V_DET4_GPIO);
#endif
...
static int papyrus_hw_power_ack(struct pmic_sess *pmsess)
{
	struct papyrus_sess *sess = pmsess->drvpar;
	struct papyrus_hw_state hwst;
	int st = 0;

	// If EPD_PWR_GOOD is not up, wait until it is asserted, or we time out
	if (sess->waiting_for_powerup) {
        	if (papyrus_synchronized_wait_for_powerup_timeout(&sess->powerup_done) == 0) {
			pr_info("papyrus: timed out waiting for EPD_PWR_GOOD signal\n");
		}
	}
	else {
		// On power down we wait; it's not critical to be synchronized
		msleep(10);
	}

	// Verify that indeed all voltages are settled
	...
	if (hwst.pg_status == 0xfa) {
		...
		// All voltages are down
		st = 0;
	}
	else {
		// At least one voltage is not settled
		st = -1;
		pr_debug("papyrus: power %s not settled yet (PG = 0x%02x)\n",
		       (sess->waiting_for_powerup) ? "up" : "down", hwst.pg_status);
	}
...
{
	disable_irq(sess->power_ready_irq);
	free_irq(sess->power_ready_irq, sess);
	gpio_free(WAKEUP_GPIO);
	gpio_free(EN_CPLD_POW_GPIO);
	gpio_free(CPLD_RESET_GPIO);
	gpio_free(PWR0_GPIO);
...
static int papyrus_probe(struct pmic_sess *pmsess, char *opt)
{
	...
	// Default VCOM value in case none is supplied
	...
	sess->power_ready_irq = OMAP_GPIO_IRQ(PWR_GOOD_GPIO);
	stat = request_irq(sess->power_ready_irq, papyrus_sess_irq_handler,
	                   IRQF_TRIGGER_RISING, "pmic-tps65185", sess);

	if (stat < 0)
	{
		pr_err("papyrus: unable to register irq %d\n", sess->power_ready_irq);
		goto free_sess;
	}

	pr_info("papyrus: registered with irq (%d)\n", sess->power_ready_irq);
...
static int papyrus_vcom_switch(struct pmic_sess *pmsess, bool state)
{
	...
	// Set the VCOM_CTRL signal to enable or disable the VCOM amplifier
	stat |= gpio_direction_output(VCOM_CTRL_GPIO, state);
