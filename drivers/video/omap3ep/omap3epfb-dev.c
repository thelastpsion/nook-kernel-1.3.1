...
static char *omap3epfb_mode_modparam = "1024x760x16x20x270x0";
...
int omap3epfb_send_recovery_signal()
{
	...
	for_each_process(p) {
		if (strncmp(p->comm, "omap-edpd", 9) == 0) {
			found = true;
			break;
		}
	}
...
static void omap3epfb_get_additional_info(struct fb_info *info,
				struct omap3epfb_addinfo *ptr)
{
	struct omap3epfb_par *par = info->par;
	ptr->empty_bq = omap3epfb_is_bq_empty(info);
	ptr->seq_finished = par->subfq.sequence_finished;
	spin_lock(&par->bfb.que.pm_lock);
	ptr->dsp_pm_sleep = par->dsp_pm_sleep;
...
static int omap3epfb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	union {
		...
		struct omap3epfb_addinfo addinfo;
//		int enable;
		int version;
...
///#TODO
		...
		case OMAP3EPFB_IO_SET_NOT_READY_DEMON:
		...
		case OMAP3EPFB_IO_GET_ADDINFO:
			omap3epfb_get_additional_info(info, &p.addinfo);
			if (copy_to_user((void __user *)arg, &p.addinfo, sizeof(struct omap3epfb_addinfo)))
				stat = -EFAULT;
			break;
