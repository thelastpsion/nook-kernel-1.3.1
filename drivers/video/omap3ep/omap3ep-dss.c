...
void omap3epdss_set_border_color(struct fb_info *info)
{
	...
	unsigned int val;
	int border_ctrl0_gpio, border_ctrl1_gpio = 0;

	if (is_avocet() && ((system_rev & 0x0F) >= BOARD_AVOCET_REV_EVT1B) &&
	                   ((system_rev & 0x0F) <= BOARD_AVOCET_REV_EVT2)) {
		border_ctrl0_gpio = 93;
		border_ctrl1_gpio = 92;
	}
	else {
		border_ctrl0_gpio = 92;
		border_ctrl1_gpio = 93;
	}
